...
H2O_smile, CO2_smile, OCOO_smile, method, solvation = "'[H]O[H]'", "'O=C=O'", "'[H]OC(=O)[O-]'", "'gfn_2'", "'gbsa'"
...
def compute_dG(cat, atom_id, which_step=1, amine_type="tert"):
    ...
    #cat_core_file_path = current_path + "/catalyst_GA/amine_rea_cores"
    ...
    De, ts3d_geom, cat3d_geom = ts_scoring(cat, ts_dummy, atom_id=atom_id, idx=(2, 2), ncpus=4, n_confs=4, cleanup=True, prot_N_patt = prot_N_patt, amine_type=amine_type)

    return De, cat3d_geom, ts3d_geom


def ts_scoring(cat, ts_dummy, atom_id=0, idx=(0, 0), ncpus=1, n_confs=10, cleanup=False, prot_N_patt = "", amine_type=""):
    ...
    ###### BAD -> DO A NOT HARDCODED VERSION

    #constr_filename = "constr_prim.inp"
    ...
    print(core_mol, Chem.MolToSmiles(core_mol))
    # ts2d = connect_cat_2d(ts_dummy, cat, atom_id, prot_N_patt = prot_N_patt)
    ...
    print("Connected mol:", Chem.MolToSmiles(ts2d))


    # for atom in ts2d.GetAtoms():
    #     print("ts2d", atom.GetSymbol(), atom.GetIdx())
    ...
    for match in ts2d.GetSubstructMatches( cat) :
        print("MATCH:", match)
    #Draw.MolsToGridImage((cat, ts2d), subImgSize=(250,250))
    #ts2d
    # if len(ts2ds) > 1:
    ...
    atoms2join = ()
    if amine_type in ["prim", "seco"]:
        atoms2join = ((n_group, co2[2]),)
    elif amine_type == "tert":
        h2o = ts2d.GetSubstructMatch(Chem.MolFromSmarts("[H]-[O]-[H]"))
        atoms2join = ((n_group, h2o[2]), (h2o[0], co2[0]) )
    else:
        print("Incorrect amine_type in ts_scoring")
        pass

    # Embed TS
    print(f"ts2d: {Chem.MolToSmiles(ts2d)}")
    print(f"ts_dummy: {Chem.MolToSmiles(ts_dummy)}")
    ...
    print(f"OUTS xtb: ts3d_energy{ts3d_energy}")
    ...
    De = (ts3d_energy - sum(frag_energies) - cat3d_energy) * hartree2kcalmol #-"frag_energies"
